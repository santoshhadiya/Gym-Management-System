...
const ManageTrainer = () => {
  ...
  const handleSaveTrainer = async (e) => {
    ...
    const payload = {
        ...formData,
        salary: { monthly: formData.salaryAmount, status: "Pending", lastPayment: "N/A" } // Structure salary object
    };
  ...
  const handleDeactivate = async (id) => {
    if (window.confirm("Are you sure you want to deactivate this trainer?")) {
      try {
         await api.patch(`/trainers/${id}/deactivate`);
         toast.info("Trainer deactivated.");
         fetchTrainers(); // Refresh list
      } catch (error) {
         console.error(error);
                ...
                <tbody className="divide-y divide-gray-100">
                    ...
                    <tr key={trainer._id} className="hover:bg-gray-50 transition-colors group">
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button onClick={() => handleDeactivate(trainer._id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors" title="Deactivate">
