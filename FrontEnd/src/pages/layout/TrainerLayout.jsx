...
const TrainerLayout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      {/* --- SIDEBAR (Desktop) --- */}
      <aside className="w-72 bg-white border-r border-gray-100 hidden lg:flex flex-col fixed h-full z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-4 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 cursor-pointer group ${
                  isActive 
                    ? 'bg-[#FEEF75] text-yellow-900 shadow-sm translate-x-1' 
        ...
        <div className="p-4 border-t border-gray-50 shrink-0">
          <button className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-bold text-red-500 hover:bg-red-50 transition-colors cursor-pointer group">
      ...
      <aside className={`fixed top-0 left-0 w-72 h-full bg-white z-50 transform transition-transform duration-300 lg:hidden shadow-2xl ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-20 flex items-center justify-between px-6 border-b border-gray-100">
           <span className="text-lg font-black text-gray-900">Menu</span>
           <button onClick={() => setIsMobileMenuOpen(false)} className="text-gray-400 hover:text-gray-600 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100">
      ...
      <div className="flex-1 flex flex-col lg:ml-72 min-h-screen transition-all duration-300">
        ...
        {/* Top Navbar */}
        <header className="h-24 bg-white/90 backdrop-blur-md border-b border-gray-100 sticky top-0 z-30 px-6 md:px-8 flex items-center justify-between shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 font-medium hidden sm:block">Let's get moving.</p>
            </div>
          </div>

          <div className="flex items-center gap-5">
             {/* Notification Bell */}
             <button className="w-11 h-11 rounded-full bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-yellow-600 hover:bg-[#FEEF75]/20 hover:border-[#FEEF75]/50 transition-all relative cursor-pointer shadow-sm group">
             ...
             <div className="flex items-center gap-3 pl-5 border-l border-gray-100 cursor-pointer group">
                <div className="text-right hidden md:block">
                   <p className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition-colors">Raj Mehta</p>
                   <p className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Senior Trainer</p>
                </div>
                <div className="w-11 h-11 rounded-full bg-[#CDE7FE] flex items-center justify-center text-blue-900 font-bold border-2 border-white shadow-md group-hover:scale-105 transition-transform">
                   RM
                </div>
             </div>
          </div>
        ...
        {/* Page Content */}
        <main className="p-6 md:p-8 flex-1 overflow-x-hidden bg-[#fafafa]">
