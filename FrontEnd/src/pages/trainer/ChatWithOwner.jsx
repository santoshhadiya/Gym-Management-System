...
const ChatWithOwner = () => {
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
              setLoading(true);
              const adminRes = await fetch(`${BACKEND_URL}/api/admin/users?role=admin`, {
  ...
  const handleAdminSelect = async (admin) => {
      const existing = conversations.find(c => c.participant?._id === admin._id);
      if (existing) {
          selectChat(existing);
      } else {
          try {
              const res = await fetch(`${BACKEND_URL}/api/chat/conversation`, {
                  ...
                  body: JSON.stringify({ receiverId: admin._id })
              });
              const newConv = await res.json();
              const fixedConv = { ...newConv, participant: admin };
  ...
  const displayList = admins.map(admin => {
      const conv = conversations.find(c => c.participant?._id === admin._id);
      return {
          ...admin,
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 no-scrollbar">
            {loading ? (
                <div className="flex flex-col items-center justify-center h-full gap-3">
                    ...
                    <p className="text-slate-400 text-sm font-medium">Syncing team...</p>
                </div>
            ) : (
                displayList.length === 0 ? (
                    <div className="text-center mt-10 text-slate-400 font-medium italic">No admins available.</div>
                ) : (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className={`text-xs truncate transition-colors ${
                                    currentChat?.participant?._id === admin._id ? 'text-slate-900 font-medium' : 'text-slate-500'
                                }`}>
                                    {admin.lastMessage}
                                </p>
                            </div>
